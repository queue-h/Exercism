...
void recite(uint8_t start_bottles, uint8_t take_down, char **song) {
  ...
  for(int bottle = start_bottles; bottle >= end_bottle; bottle--) {
    if(bottle > 1) {
      sprintf(*song++, FIRST_LINE[2], bottle, bottle);
    } else {
      sprintf(*song++, FIRST_LINE[bottle], bottle, bottle);
    }
    if(bottle - 1 > 2) {
      sprintf(*song++, SECOND_LINE[3], bottle - 1);
    } else {
      sprintf(*song++, SECOND_LINE[bottle], bottle - 1);
